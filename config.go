...
const PubsubTopicID = "fill-session-details"
...
func init() {
	...
	//
	DB, err = configureDatastoreDB("vyfe-api")
	// [END datastore]
	...
	// bucket name.
	//
	StorageBucketName = "staging-local"
	...
	// production.
	//
	OAuthConfig = configureOAuthClient("468896191511-ngk3oercplt5fp4335asqkvccvt5pdho.apps.googleusercontent.com", "sxdxTlZUllmE6QXYa8aFKpd5")
	// [END auth]
	...
	//
	PubsubClient, err = configurePubsub("vyfe-api")
	// [END pubsub]
...
func configureDatastoreDB(projectID string) (SessionDatabase, error) {
...
func configurePubsub(projectID string) (*pubsub.Client, error) {
	if _, ok := DB.(*memoryDB); ok {
		return nil, errors.New("Pub/Sub worker doesn't work with the in-memory DB " +
			...
			"database in vyfe-api/config.go first (e.g. MySQL, Cloud Datastore, etc)")
	}
